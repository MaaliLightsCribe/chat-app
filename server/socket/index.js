...
const initSocket = (server, corsOptions) => {
  ...
  io.on('connection', socket => {
    ...
    socket.on('UPDATE_MESSAGE_STATUS', ({ type, readerId, messageSender }) => {
      // messageSender Message quilt readerId Have read
    ...
    socket.on('ENTER_CHAT_ROOM', roomData => {
      ...
      // New
      socket.join(roomId)
      // People other than them receive the message
    ...
    socket.on('ROOM_CREATED', ({ name, creator, invitedUser }) => {
      invitedUser.forEach(invitedUser => {
        ...
        // The invited person will notify it online
        if (socketId) {
          socket.to(socketId).emit('INVITED_TO_ROOM', { message: `${creator} added you in ${name} chatroom`})
        }
      })
    })
  })
