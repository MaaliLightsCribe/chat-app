...
const getUnreadCount = asyncHandler(async (type, from, to) => {
  ...
  const messageReaders = await Message
    .find({ sender: { $ne: from } }) // sender Not your own message
    .all('users', filter)
    .select(['readers'])
    .sort({ createdAt: -1 })
    .lean()
  
  // readers There is no own own id
...
const updateMessageReadStatus = asyncHandler(async (req, res) => {
  try {
    // chatId Message quilt userId Have read
    const { userId } = req.params
    const { type, chatId } = req.query
    ...
    // Remove everything chat middle sender Not your own message
    const messages = await Message
      .find({ sender: { $ne: userId }})
      .all('users', filter)
      .sort({ createdAt: 1 })

    // Obtain message and reader Corresponding value
    const messageReaderMap = messages.reduce((prev, curr) => {
      return {...prev, [curr._id.toHexString()]: curr.readers }
    }, {})

    // examine userId Does it exist? readers inside -> If there is no existence
    Object.entries(messageReaderMap).forEach(([key, value]) => {
      const userHasRead = value.indexOf(userId) > -1
      if (!userHasRead) messageReaderMap[key].push(userId) // Add without reading it yet
    })
