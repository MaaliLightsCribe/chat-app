...
function ChatRoom() {
  ...
  const checkIsChatting = useCallback(
    (messageData) => {
      // Check if you are in the dialogue
  ...
  // socket Receive the message -> Update dialogue message status
  useEffect(() => {
    if (messageData) {
      console.log('=== socket Receive the message ===', messageData);
      // Check if you are in the dialogue
      const isChatting = checkIsChatting(messageData);
      // Yes, the update message status is read by yourself
      if (isChatting) {
        // Update yourself
        updateSelfMessageStatus(messageData);
        // renew API & other side
  ...
  // socket receive message update status PASS
  useEffect(() => {
    if (messageReadStatus) {
      const { type, readerId, toId: receiveRoomId } = messageReadStatus;
      // Check whether the reader is in the dialogue
