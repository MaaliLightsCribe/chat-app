...
function ChatRoomInput({ setChatMessages }) {
  ...
  const handleInputSubmit = (e) => {
    ...
    postUserMessage(
      ...
      (data) => {
        // Update yourself room message
        setChatMessages((prev) => [...prev, { ...data.data, avatarImage: user.avatarImage }]);

        // use socket Notify the other party in real time
  ...
  useEffect(() => {
    if (typingNotify) {
      ...
      setShowNotify(typing && isChatting); // Only chat typing Be displayed
    } else {
      setShowNotify(false);
    }
  }, [typingNotify, chatId]);
